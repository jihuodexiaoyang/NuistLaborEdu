    ...
    $.extend({
        ...
        operate: {
            ...
            choose: function(id) {
                // 检查是否选择了课程
                if (!id) {
                    $.modal.alertError("请选择要选的课程！");
                    return;
                }
                // 设置表格
                table.set();
                // 弹出确认框
                $.modal.confirm("确定选中该条" + table.options.modalName + "课程吗？", function() {
                    // 根据选择的课程id构建请求URL
                    var url = $.common.isEmpty(id) ? table.options.chooseUrl : table.options.chooseUrl.replace("{id}", id);
                    // 发送选课请求
                    if (table.options.type == table_type.bootstrapTreeTable) {
                        // 如果表格类型为 bootstrapTreeTable，则发送 GET 请求
                        $.operate.get(url);
                    } else {
                        // 否则，构建请求数据
                        var data = { "ids": id };
            ...
            initorg: function() {
                table.set();
                $.modal.confirm("确定同步" + table.options.modalName + "吗？", function() {
                    $.modal.loading("正在初始化数据，请稍后...");
                    $.post(table.options.initUrl, {status:"0"}, function(result) {
                        if (result.code == web_status.SUCCESS) {
                            $.modal.alertSuccess(result.msg);
                            $.treeTable.refresh();
                        } else if (result.code == web_status.WARNING) {
